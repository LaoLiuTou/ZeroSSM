...
public class SqlServerMain {
		...
		public static List getTables(){
	        ...
	        try {
		        ...
		        result = stmt.executeQuery("SELECT Name FROM SysObjects Where XType='U' ORDER BY Name"); //执行SQL 语句，查询数据库  
		...
		public static String getPKey(String tableName){
	        ...
	        try {
		        ...
		        con = DriverManager.getConnection(jb.getDburl(),jb.getDbuser(),jb.getDbpassword());  
		        stmt = con.createStatement();  
		        result = stmt.executeQuery("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_NAME='"+tableName+"'");
