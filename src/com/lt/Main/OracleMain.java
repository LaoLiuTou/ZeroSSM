...
public class OracleMain {
	...
	public static List getTables(){
        ...
        try {
	        ...
	        result = stmt.executeQuery("select table_name from user_tables"); //执行SQL 语句，查询数据库  
		...
		//tableList.add("SMS_IN");
		//tableList.add("SMS_OUT");
		//tableList.add("EVALUATE");
		//tableList.add("SYSTEMUSER");
		//tableList.add("CC20");
		//tableList.add("NEWS_COMMENT");
		tableList.add("NEWS");
		//tableList.add("CC20_PP");
	...
	public static String getPKey(String tableName){
        ...
        try {
	        ...
	        con = DriverManager.getConnection(jb.getDburl(),jb.getDbuser(),jb.getDbpassword());  
	        stmt = con.createStatement();  
	        result = stmt.executeQuery("select cu.* from user_cons_columns cu, user_constraints au " +
